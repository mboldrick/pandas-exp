...
def process_dates_and_job_no(file_path):
    ...
    # Convert 'date' column to datetime objects and format to 'YYYY-MM-DD', coercing errors
    # df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.strftime("%Y-%m-%d")
    # Vectorized conversion of 'date' column to datetime objects
    df["date"] = pd.to_datetime(df["date"], errors="coerce")

    # Formatting the datetime objects to 'YYYY-MM-DD' format
